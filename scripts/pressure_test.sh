...
log "note on fill-p99: don't read a high fill-p99 as pipeline queueing without checking the trend"
log "across steps first. This benchmark's order flow is synthetic and non-price-reactive (every"
log "worker picks a random price near mid-price regardless of the current book), so it can easily"
log "generate more resting liquidity than crossing volume at a given instant — a resting order can"
log "then wait a genuinely long time to be crossed by chance, which is a property of this test's"
log "order-flow model, not gateway/engine latency. The tell: a REAL pipeline bottleneck gets worse"
log "as load increases; this effect gets BETTER (more concurrent flow crosses resting orders sooner)."
log "If fill-p99 is high AND flat-or-worsening as users scale up, that's worth investigating as a"
log "real bottleneck; if it's high but improves as users scale up, it's most likely this artifact."
